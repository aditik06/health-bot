// Adapted from Motion Primitives (motion-primitives.com/docs/text-effect),
// stripped of TypeScript and its Tailwind `cn()` helper - this project isn't
// on Tailwind, so the handful of utility classes it relies on (`block`,
// `inline-block`, `whitespace-pre`, `sr-only`) are defined once in
// styles.css instead of pulled in as a framework.
...
const AnimationComponent = React.memo(({ segment, variants, per, segmentWrapperClassName }) => {
    ...
    return <span className={joinClasses(defaultWrapperClassName, segmentWrapperClassName)}>{content}</span>;
});
...
function hasTransition(variant) {
    return !!variant && typeof variant === 'object' && 'transition' in variant;
}

function createVariantsWithTransition(baseVariants, transition) {
...
export function TextEffect({
    ...
    const baseVariants = preset
        ...
        : { container: defaultContainerVariants, item: defaultContainerVariants };
